"""LeWM training objective for TEP latent dynamics."""
...
class LeWMLoss(nn.Module):
    """Combine next-embedding prediction MSE with the LeWM SIGReg term."""

    def __init__(
        self, sigreg_weight: float = 0.09, knots: int = 17, num_proj: int = 1024
    ):
        """Configure the single LeWM loss weight and SIGReg approximation."""
        super().__init__()
        if sigreg_weight < 0:
    ...
    def forward(self, output: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        """Return total, prediction and regularization losses for one batch."""
