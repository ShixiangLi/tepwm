    ...
    for index, scenario_type in enumerate(scenario_types):
        ...
        if scenario_type in {"single_action", "multi_action", "boundary_action"}:
            config["action_events"] = _sample_action_events(
                ...
                required_sp, rng, force_boundary=scenario_type == "boundary_action",
            )
    ...
    *,
    ...
    for event_index in range(event_count):
        ...
        for sp in map(int, selected):
            values[f"SP{sp}"] = _sample_action_value(
                config, sp, current[sp], rng,
                force_boundary=force_boundary and event_index == 0 and sp == required_sp,
            )
...
def _sample_action_value(config: Mapping[str, Any], sp: int, current: float,
                         rng: np.random.Generator, *, force_boundary=False) -> float:
    ...
    if force_boundary or rng.random() < float(config.get("boundary_probability", 0.1)):
...
def _sample_mode_events(generation, current_mode, duration, rng):
    ...
    event_time = _randint([bounds[0], min(bounds[1], latest)], rng)
